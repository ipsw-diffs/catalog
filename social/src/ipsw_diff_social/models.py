...
def require_schema_version(value: object, field: str) -> None:
    if type(value) is not int or value != 1:
        raise DataError(f"{field} must be the integer 1")
...
class CatalogEntry:
    ...
    # Preserve every field before URL/text normalization, including provenance and integrity.
    canonical_row: str

    @classmethod
    def from_value(cls, value: object) -> CatalogEntry:
        data = _mapping(value, "entry")
        require_schema_version(data.get("schema_version"), "entry.schema_version")
        ...
        return cls(
            ...
            canonical_row=json.dumps(data, sort_keys=True, separators=(",", ":"), allow_nan=False),
        )

...
TITLE_TOP = 675
MAX_TEXT_WIDTH = 1400
# JPEG keeps the card well under Bluesky's 1 MB image limit; the brand art alone is ~900 KB.
JPEG_QUALITY = 88
...
def card_alt_text(entry: CatalogEntry, names: ReleaseNames, facts: list[DiffFact]) -> str:
    """Describe the card's text for screen readers."""
    sentences = [
        f"ipsw-diff card: {release_title(entry, names)}",
        f"Builds {entry.previous.build} to {entry.current.build} on {entry.device}",
    ]
    if facts:
        sentences.append(", ".join(f"{fact.count:,} {fact.area} {fact.change}" for fact in facts))
...
def render_card(
    ...
    image.convert("RGB").save(output_path, format="JPEG", quality=JPEG_QUALITY, optimize=True)
